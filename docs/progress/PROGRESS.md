...
- 最后更新：2026-06-04（M0 ✅ + M1 主体 ✅ + **M2 主体 ✅** + M3 渲染/预览/格式化 ✅ + M4 一次性/流式 agent ✅；自主连续推进 0015–0030 全部编译/CI/测试层验证并推送）
- 当前里程碑：**M2 可日用基本成形**（文本力量/搜索/文件 CRUD+监听/分屏/全部保存/标签拖拽已交付）；M4 流式 agent 已落地（端到端待装 agent）
...
**剩余（需你的环境才能负责任完成 + 验证）**：
- ⛔ **LSP**：需真实语言服务器子进程 + 运行 app 验证补全/诊断；盲写 `@codemirror/lsp-client` 桥接无服务器可测。
- ⛔ **M4 端到端**：流式已落地，但真实对话/权限/diff 审查需本地装 ACP agent（如 `claude-agent-acp`）+ 运行 app 验握手。
- ⛔ **行内 Live Preview**（CM 装饰，Obsidian 式）：需显示环境肉眼调视觉(守"开箱即美"高线,盲做有损品质)。
- ⛔ **输入→上屏 端到端延迟实测**：文档模型层门禁已有(0030)；真实绘制延迟需显示环境 + Playwright/tauri-driver。
