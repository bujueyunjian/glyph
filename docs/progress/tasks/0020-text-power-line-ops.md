...
### 验证结果
- ✅ `pnpm test`：26/26（新增 8 例行变换）。
- ✅ 完整 `pnpm check` 全绿。
- ✅ `pnpm build && pnpm perf:budget`：首屏 127.7KB / 170KB（lineOps 纯函数 +~0.6KB，红线守住）。
- ⏳ GUI 行为（命令面板触发 → 编辑器内变换选区/整篇）待显示环境自验。

### 遗留问题（后续刀）
- **前缀 / 后缀 / 前后缀**：纯函数 `addPrefix`/`addSuffix` 已就绪并测，但需命令面板输入框 UI 才能接入（下一刀）。
- **多选区批量**：当前作用主选区 / 整篇；多光标批量作用是下一刀。
- 列编辑、join/split 等扩展行操作。

### 下一步
- prefix/suffix 接输入框；多选区批量作用；然后推进 M2 其余（跨文件搜索 / 文件树 CRUD+监听 / 分屏）。
