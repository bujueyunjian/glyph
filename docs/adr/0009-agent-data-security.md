...
(如 `claude-code-acp`)做本地子进程,经 stdio 转发提问与回答。这条链路天然涉及**数据外发**——
...
- ✅ **作用域收敛已落地(0046)**:agent 的 cwd 现钉到打开的工作区根(`resolve_cwd`:传入工作区根优先,
  缺省才回退进程目录),不再默认暴露打包 app 的启动目录(可能是 `/`)。由 `resolves_cwd_prefers_workspace_root` 测试守卫。
- ⚠️ **同意不可在 UI 内撤销(v1)**:清除需手动清 localStorage;v2 在设置面板提供开关。
- 🔜 **后续**:agent 命令来源加显式「仅用户配置」校验;细粒度能力授权弹窗;同意状态进设置面板可见可撤销。
